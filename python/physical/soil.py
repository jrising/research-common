...
class HWSDSoil(GlobalBinaryGrid, StaticFeature):
    ...
    def __init__(self, hwsdroot, is_gzipped=True, error_handler=print_error_handler):
        self.hwsdroot = hwsdroot
        if is_gzipped:
            hwsd = gzip.open(os.path.join(hwsdroot, 'HWSD_RASTER/hwsd.bil.gz'))
        else:
            hwsd = open(os.path.join(hwsdroot, 'HWSD_RASTER/hwsd.bil'))
    ...
    def cache_soilinfo(self):
        self.soildefs = {}
        with gzip.open(os.path.join(self.hwsdroot, 'HWSD/HWSD_DATA.txt.gz')) as f:
            reader = csv.reader(f)
            for row in reader:
                self.soildefs[row[0]] = row

        self.soilnames = {'symbols': get_dict(os.path.join(self.hwsdroot, 'HWSD/D_SYMBOL85.txt.gz'), 2, 1),
                          'classes': get_dict(os.path.join(self.hwsdroot, 'HWSD/D_USDA_TEX_CLASS.txt.gz'), 0, 1)}
    ...
    def get_ll_plus(self, latitude, longitude, plus=True):
        ...
        if plus and vals is None:
            ...
            if vals is None and latitude < 90 - self.sizey:
                vals = self.getll(latitude + self.sizey, longitude)
            if vals is None:
                self.error_handler("Could not find a neighboring soil")
            else:
                self.error_handler("Using a neighboring soil")
    ...
    def get_soilinfo(self, zstr):
        ...
        self.error_handler("Soil Error: soil type not found: " + zstr)
        return None
    ...
    def soil_to_texture(self, zstr):
        ...
        return dict(top=dict(sand=floatnone(row[24]), silt=floatnone(row[25]), clay=floatnone(row[26]), carb=floatnone(row[30]), calc=floatnone(row[36]), gyps=floatnone(row[37])),
                    bot=dict(sand=floatnone(row[41]), silt=floatnone(row[42]), clay=floatnone(row[43]), carb=floatnone(row[47]), calc=floatnone(row[53]), gyps=floatnone(row[54])))
    ...
    def get_properties(self, row):
        ...
        try:
            ...
            self.error_handler("Soil Error: " + str(sys.exc_info()))
            return None
        ...
        if self.soilnames is None:
            soilname = get_val(os.path.join(self.hwsdroot, 'HWSD/D_SYMBOL85.txt.gz'), soilsym, 2, 1)
            topname = get_val(os.path.join(self.hwsdroot, 'HWSD/D_USDA_TEX_CLASS.txt.gz'), topnum, 0, 1)
            botname = get_val(os.path.join(self.hwsdroot, 'HWSD/D_USDA_TEX_CLASS.txt.gz'), botnum, 0, 1)
        else:
